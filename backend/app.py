...
# The frontend is served by this same Flask app now, so same-origin requests
# never touch CORS. This stays configurable only for the rare case where a
# separate frontend origin needs API access.
CORS(app, origins=os.environ.get("CORS_ALLOWED_ORIGINS", "*"), supports_credentials=True)
...
# ---------------------------------------------------------------------------
# Page routes — Flask serves the whole frontend from templates/, so this is
# the only Web Service Render needs to run. Endpoint names below (home_page,
# login_page) are referenced by templates via url_for(), so don't rename
# without updating index.html / login.html too.
# ---------------------------------------------------------------------------
@app.get("/")
def home_page():
...
@app.get("/member")
def member_page():
    return render_template("pages/member-dashboard.html")
...
# ---------------------------------------------------------------------------
# Error handling — API requests get JSON, page requests get a rendered page.
# Falls back to plain text if the error template itself is missing so a
# broken template can never turn into an unhandled 500 loop.
# ---------------------------------------------------------------------------
def _wants_json():
